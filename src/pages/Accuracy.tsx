...
const Accuracy = () => {
  ...
  const runSync = async (silent = false) => {
    ...
    if (res.ok) {
      if (!silent) {
        toast({
          title: "Sync complete",
          description: `Pulled ${res.officialMatches ?? 0} official IPL scorecard${res.officialMatches === 1 ? "" : "s"}. Predictions re-graded.`,
        });
      }
  ...
  return (
    <PageLayout
      ...
      subtitle="Every prediction is saved and graded from the latest official IPL scorecards, with Cricsheet used as a verified backup."
    >
      {/* Sync banner */}
        ...
        <CardContent className="flex flex-col gap-3 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="text-sm">
            ...
            <div className="text-xs text-muted-foreground">
              Click below to pull the latest official IPL scorecards and grade completed matches.
            </div>
          </div>
