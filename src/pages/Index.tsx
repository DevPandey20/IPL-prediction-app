...
const Index = () => {
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => {
        ...
        if (data?.last_run_at) setLastRun(data.last_run_at);
      });
  }, []);
  ...
  const handleSync = async () => {
    ...
    if (res.ok) {
      toast({
        title: "Sync complete",
        description: `Pulled ${res.officialMatches ?? 0} official IPL scorecard${res.officialMatches === 1 ? "" : "s"}.`,
      });
      ...
      <div className="relative -mx-4 -mt-8 mb-6 overflow-hidden bg-gradient-to-br from-primary via-primary to-primary/85 px-4 py-12">
        ...
        <div className="relative mx-auto max-w-3xl text-center text-primary-foreground">
          ...
          <p className="mt-2 text-primary-foreground/80">
            Latest official IPL scorecards first • Cricsheet as backup
          </p>
          <div className="mt-4 flex items-center justify-center gap-3 flex-wrap">
            <Button size="sm" variant="secondary" onClick={handleSync} disabled={syncing} className="gap-2">
              <RefreshCw className={`h-4 w-4 ${syncing ? "animate-spin" : ""}`} />
              {syncing ? "Syncing latest scores..." : "Sync Latest Data"}
            </Button>
            {lastSync && (
              <span className="text-xs text-primary-foreground/70">
                Latest match in DB: {lastSync}
                {lastRun && ` · synced ${new Date(lastRun).toLocaleString()}`}
              </span>
            )}
          </div>
          <p className="mt-2 text-[11px] text-primary-foreground/60">
            Pulls from the official IPL score feed, including live/current-season matches, then verifies history with Cricsheet.
          </p>
        </div>
      </div>
