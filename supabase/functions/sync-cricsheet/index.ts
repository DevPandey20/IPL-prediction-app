// Sync IPL match data from two sources:
// - Official IPL score feeds first for live/latest scorecards
// - Cricsheet as the historical verified ball-by-ball fallback
// - Aggregates per-player batting/bowling stats -> player_match_stats
// - Stores deliveries -> ball_by_ball (for true H2H matchups)
...
const OFFICIAL_FEED = "https://scores.iplt20.com/ipl/feeds";
const OFFICIAL_COMP_IDS = [284, 203, 148, 107, 60, 33, 24, 8]; // IPL 2026 -> 2019
...
function parseJsonp(text: string) {
  ...
  if (start < 0 || end <= start) throw new Error("Invalid JSONP feed");
  return JSON.parse(text.slice(start + 1, end));
}

async function fetchJsonp(url: string) {
  const r = await fetch(url, { headers: { "user-agent": "Mozilla/5.0", referer: "https://www.iplt20.com/" } });
...
function officialWinner(m: any): string | null {
  ...
  if (winningId && winningId === String(m.FirstBattingTeamID)) return m.FirstBattingTeamName;
  if (winningId && winningId === String(m.SecondBattingTeamID)) return m.SecondBattingTeamName;
  const comment = String(m.Commentss ?? m.Comments ?? "");
  return comment.includes(" Won by") ? comment.split(" Won by")[0].trim() : null;
}

async function syncOfficialIpl(supabase: any, lastDate: Date) {
  let officialMatches = 0;
  let officialRows = 0;
  let officialBalls = 0;
  let newestDate = lastDate;

  for (const cid of OFFICIAL_COMP_IDS) {
    let schedule: any[] = [];
    try {
      const json = await fetchJsonp(`${OFFICIAL_FEED}/${cid}-matchschedule.js`);
      schedule = json?.Matchsummary ?? [];
    } catch (e) {
      console.warn(`Official schedule ${cid} failed:`, (e as Error).message);
      continue;
    }

    for (const m of schedule) {
      const status = String(m.MatchStatus ?? "").toLowerCase();
      if (status !== "post" && status !== "live") continue;
      const dateStr = m.MatchDate;
      if (!dateStr) continue;
      const matchDate = new Date(dateStr);
      const isCurrentSeason = cid === OFFICIAL_COMP_IDS[0];
      if (!isCurrentSeason && matchDate <= lastDate) continue;

      const matchId = `ipl-${cid}-${m.MatchID}`;
      const winner = officialWinner(m);
      await supabase.from("match_results").upsert({
        match_id: matchId,
        match_date: dateStr,
        season: String(cid),
        venue: m.GroundName ?? "Unknown",
        city: m.city ?? "",
        team1: m.FirstBattingTeamName ?? m.HomeTeamName ?? "Unknown",
        team2: m.SecondBattingTeamName ?? m.AwayTeamName ?? "Unknown",
        winner,
        result: m.Commentss ?? m.Comments ?? null,
        synced_at: new Date().toISOString(),
      });

      const rows = new Map<string, any>();
      const balls: BallRow[] = [];
      for (const innNo of [1, 2]) {
        let innings: any;
        try {
          const json = await fetchJsonp(`${OFFICIAL_FEED}/${m.MatchID}-Innings${innNo}.js`);
          innings = json?.[`Innings${innNo}`];
        } catch {
          continue;
        }
        if (!innings) continue;
        const extra = innings.Extras?.[0] ?? {};
        const battingTeam = extra.BattingTeamName ?? (innNo === 1 ? m.FirstBattingTeamName : m.SecondBattingTeamName) ?? "Unknown";
        const bowlingTeam = extra.BowlingTeamName ?? (innNo === 1 ? m.SecondBattingTeamName : m.FirstBattingTeamName) ?? "Unknown";
        const ensure = (player: string, ownTeam: string, opponent: string) => {
          const player_name = cleanName(player);
          const key = player_name.toLowerCase();
          if (!rows.has(key)) rows.set(key, { match_id: matchId, player_name, own_team: ownTeam, opponent, runs: 0, balls: 0, wickets: 0, overs_bowled: 0, runs_conceded: 0, economy: null, venue: m.GroundName ?? "Unknown", city: m.city ?? "", match_date: dateStr, season: String(cid) });
          return rows.get(key);
        };

        for (const b of innings.BattingCard ?? []) {
          const row = ensure(b.PlayerName ?? "Unknown", battingTeam, bowlingTeam);
          row.runs = n(b.Runs);
          row.balls = n(b.Balls);
        }
        for (const b of innings.BowlingCard ?? []) {
          const row = ensure(b.PlayerName ?? "Unknown", bowlingTeam, battingTeam);
          row.wickets = n(b.Wickets);
          row.overs_bowled = n(b.Overs);
          row.runs_conceded = n(b.Runs);
          row.economy = b.Economy === "" || b.Economy == null ? null : n(b.Economy);
        }
        for (const h of innings.OverHistory ?? []) {
          balls.push({
            match_id: matchId,
            innings: innNo,
            over_num: n(h.BallName || String(h.OverNo ?? "0")),
            batter: cleanName(h.BatsManName ?? "Unknown"),
            bowler: cleanName(h.BowlerName ?? "Unknown"),
            non_striker: null,
            batter_team: battingTeam,
            bowler_team: bowlingTeam,
            runs_batter: n(h.ActualRuns),
            runs_extras: n(h.Extras),
            runs_total: n(h.ActualRuns) + n(h.Extras),
            is_wicket: String(h.IsWicket) === "1",
            dismissal_kind: h.WicketType || null,
            player_out: String(h.IsWicket) === "1" ? cleanName(h.BatsManName ?? "") : null,
            venue: m.GroundName ?? "Unknown",
            match_date: dateStr,
            season: String(cid),
          });
      ...
      const statRows = Array.from(rows.values()).filter((r) => r.player_name && r.player_name !== "Unknown");
      if (statRows.length > 0) {
        const { error } = await supabase.from("player_match_stats").upsert(statRows, { onConflict: "match_id,player_name" });
        if (error) console.error(`Official stat upsert failed for ${matchId}:`, error.message);
        else officialRows += statRows.length;
      }
      if (balls.length > 0) {
        await supabase.from("ball_by_ball").delete().eq("match_id", matchId);
        for (let i = 0; i < balls.length; i += 500) {
          const { error } = await supabase.from("ball_by_ball").insert(balls.slice(i, i + 500));
          if (error) console.error(`Official ball insert failed for ${matchId}:`, error.message);
          else officialBalls += Math.min(500, balls.length - i);
        }
      }
      officialMatches += 1;
  ...
  return { officialMatches, officialRows, officialBalls, newestDate };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log("Syncing official IPL feed first...");
    const official = await syncOfficialIpl(supabase, lastDate);
    console.log(
      `Official IPL: ${official.officialMatches} matches / ${official.officialRows} rows / ${official.officialBalls} balls`,
    );
    ...
    if (forceRecent) {
      ...
      await supabase.from("sync_state").upsert({
        key: "cricsheet_ipl",
        last_synced_date: official.newestDate.toISOString().slice(0, 10),
        last_run_at: new Date().toISOString(),
        matches_synced: official.officialMatches,
        notes: `Official IPL latest sync: ${official.officialMatches} matches / ${official.officialRows} rows / ${official.officialBalls} balls. Graded: ${gradedCount}.`,
      });
      return new Response(
        JSON.stringify({
          ok: true,
          ...
          new_player_rows: 0,
          new_balls: 0,
          official_matches: official.officialMatches,
          official_player_rows: official.officialRows,
          official_balls: official.officialBalls,
          graded_predictions: gradedCount,
          newest_match_date: official.newestDate.toISOString().slice(0, 10),
        }),
    ...
    let newRows = 0;
    let newBalls = 0;
    let newestDate = official.newestDate;
    ...
    await supabase.from("sync_state").upsert({
      ...
      notes: `Official IPL: ${official.officialMatches} matches / ${official.officialRows} rows / ${official.officialBalls} balls. Cricsheet: ${newMatches} matches / ${newRows} rows / ${newBalls} balls. Graded: ${gradedCount}.`,
    });
